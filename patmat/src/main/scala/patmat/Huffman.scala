...
object Huffman {
   ...
   *   }
   */
    def times(chars: List[Char]): List[(Char, Int)] = (Map[Char, Int]() /: chars) {
   ...
   */
    def makeOrderedLeafList(freqs: List[(Char, Int)]): List[Leaf] = freqs map { a => Leaf(a._1, a._2) } sortBy { weight _ }
   ...
   */
    def combine(trees: List[CodeTree]): List[CodeTree] = trees match {
      ...
      case x :: y :: ts   => makeCodeTree(x, y) :: ts sortBy { weight _ } // improve perf
    }
   ...
   */
    def until(p: List[CodeTree] => Boolean, f: List[CodeTree] => List[CodeTree])(z: List[CodeTree]): List[CodeTree] = if (p(z)) z else until(p, f)(f(z))
