 ...
 */
 @RunWith(classOf[JUnitRunner])
   ...
   */
  test("sum of a few numbers 1") {
    assert(sum(List(1,2,0)) === 3)
  }

  test("sum of a few numbers 2") {
    assert(sum(List(2,1,0)) === 3)
  }

  test("sum of a few numbers 3") {
    assert(sum(List(0,1,2)) === 3)
  }
  ...
  test("max of a few numbers 3") {
    assert(max(List(2, 3, 7)) === 7)
  }
  ...
  test("max of a few negative and positive numbers") {
    assert(max(List(-2, 3, -7)) === 3)
  }
