...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    (SortedMap[Char,Int]() ++ w.toLowerCase.groupBy(identity).map { case (c,cs) => (c, cs.length) }).toList
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    y.foldLeft(SortedMap[Char,Int]() ++ x) { case (map, (guess, count)) =>
      val temp = map(guess) - count
      if (temp != 0) map.updated(guess, temp)
      else map - guess
    }.toList
  }
